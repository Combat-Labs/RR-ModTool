...
public class MaterialTarget : IEquatable<MaterialTarget>
{
	...
	public static Dictionary<MaterialTarget, CachedMaterial> CacheCount = new Dictionary<MaterialTarget, CachedMaterial>();
	...
	public void SetFloat(string shaderVariableName, float value)
	{
		...
		if (ValidMaterial(MaterialIndex))
		{
			if(shaderVariableName == "_Transparency")
			{
				...
				Renderer.sharedMaterials[MaterialIndex].SetInt("_SrcBlend", (int)UnityEngine.Rendering.BlendMode.SrcAlpha);
				Renderer.sharedMaterials[MaterialIndex].SetInt("_DstBlend", (int)UnityEngine.Rendering.BlendMode.OneMinusSrcAlpha);
