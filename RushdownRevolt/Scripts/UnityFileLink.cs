...
public class UnityFileLink<T> : UnityFileLink where T : UnityEngine.Object
{
    ...
    [SerializeField] private string _FILE_PATH; // File path is authoritative for regenerating the reference

    public T obj
    {
        get { return (reference == null) ? null : (T)reference; }
    }
            ...
            if (reference == null)
            {
                ...
                if (objectFile == null)
                {
                    Debug.LogErrorFormat(
                        "UNITY FILE ERROR, unable to automatically repair link to: {0}, from object {1}", _FILE_PATH,
                        ownerFilePath);
                }
                else
                {
